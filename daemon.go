 ...
 * @Version: 1.0
 * @Autor: solid
 * @Date: 2022-01-14 17:38:33
 * @LastEditors: solid
 * @LastEditTime: 2022-01-14 18:47:57
 */
package xdaemon
...
//守护进程
type Daemon struct {
	Exec        interface{} //执行的命令
	LogPath     string      //日志文件路径
	MaxCount    int         //循环重启最大次数,若为0则无限启动
	MaxError    int         //连续启动失败或异常退出的最大次数, 超过此数, 守护进程退出, 不再重启子进程,若为0则无限重试
	MinExitTime int64       //子进程正常退出的最小时间(秒). 小于此时间则认为是异常退出
	runIdx      int         //运行时调用background的次数
}

var DefaultdefaultDaemon = &defaultDaemon

const ENV_NAME = "XW_DAEMON_IDX"

var defaultDaemon Daemon = Daemon{
	Exec:        os.Args,
	LogPath:     "",
	MaxCount:    0,
	MaxError:    3,
...
func (d *Daemon) Background(isExit bool) (*exec.Cmd, error) {
	//判断子进程还是父进程
	d.runIdx++
	...
	if d.runIdx <= envIdx { //子进程, 退出
		return nil, nil
	}

	//设置子进程环境变量
	env := os.Environ()
	env = append(env, fmt.Sprintf("%s=%d", ENV_NAME, d.runIdx))
	//启动子进程
	cmd, err := d.startProc(d.Exec, env, d.LogPath)
...
func (d *Daemon) startProc(cmdExec interface{}, env []string, logFile string) (*exec.Cmd, error) {
	var cmd *exec.Cmd
	if runtime.GOOS != "linux" {
		...
		cmd.SysProcAttr = &syscall.SysProcAttr{CmdLine: fmt.Sprintf(`/c %v`, cmdExec), HideWindow: true}
	} else {
		cmd = &exec.Cmd{
			Path: "/bin/bash",
			Env:  env,
		}
		cmd.SysProcAttr = &syscall.SysProcAttr{CmdLine: fmt.Sprintf(`-c %v`, cmdExec)}
	}
	if logFile != "" {
...
func DaemonProcess(path string, logPath string) {
	daemon := &Daemon{
		Exec:    path,
		LogPath: logPath,
	}
